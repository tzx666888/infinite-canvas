...
- 生产发布固定按“备份、版本号、提交、推送、构建、验证、上线、保留回滚”顺序执行；不得跳过线上备份、实际页面验证或回滚目标记录。
...
- 画布 Agent 的平台文本/视觉编排固定优先使用 `gpt-5.6-sol`，失败时切换 `doubao-seed-2-1-pro-260628`；不得使用 DeepSeek 处理画布 Agent 或参考图任务。
- 视频提示词按完整度路由：短提示词可用内置带货结构补全，中等提示词只轻量补全，超过 600 字或已包含时间轴/完整约束时必须优先原样保留用户原文；所有“不要人物/烟雾/旋转”等硬约束永远高于内置模板，不得被截断或改写。
- 零参考图的纯文生视频与参考图视频必须使用同一套短/中/长提示词路由；短带货指令必须在提交上游前完成补强，普通非商业提示词保持原样。
- 短带货视频 Hook 必须按成片时长硬路由：10 秒完整占用前 2 秒并在 2.00 秒硬切商品，15 秒 MiniMax H3 完整占用前 3 秒并在 3.00 秒硬切商品；不得提前揭示商品后用静态画面凑满 Hook 时长。普通手持、正常走路、慢推镜、静态特写或常规展示不算 Hook，Omni 与 MiniMax H3 必须使用同一套路由与验收规则。
- 商业视频和提示词库默认保持全球中性；只有用户或逐题引导明确指定国家、城市、语言、货币或平台时才做对应本地化，不得在共享模板中硬编码单一国家、语言、货币、平台或城市。
...
- 修复模型路由、超时或参考素材传输问题时，只修改故障链路；不得顺带改变各模型已经确定的模型 ID、时长、分辨率、比例、声音或参考素材数量参数。
