...
- 简短带货提示词默认从商品可见细节或真实使用动作自然开场，商品允许首帧出现；10 秒与 15 秒分别用约 2 秒与 3 秒完成开场，再连贯进入演示和收束。不得强加与商品无关的事故、道具、滑倒或固定甩镜。用户明确给出的开场和禁令优先；显式选择“爆款 Hook”也必须围绕商品本身，不能套随机事故模板。Omni 与 MiniMax H3 使用同一原则。
