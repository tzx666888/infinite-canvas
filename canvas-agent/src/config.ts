...
客户要印尼 TikTok Shop 带货视频、UGC、Hook、印尼语口播、MiniMax H3 提示词或成片验收时触发。读取当前画布商品图和已验证资料，先建立 Product Truth Sheet（visual_observed / user_supplied / verified_product_data / unknown），提取品类、完整品名、SKU/规格/颜色/口味/数量、包装结构、logo/标签位置和逐字包装文字；看不清写“包装文字不可辨”，多个 SKU、资料矛盾、语言变体不明或核心卖点无证据时先提问并暂停，不得编造功效、价格、折扣、销量、评价、库存紧迫感或前后对比。
先给 3 个不同 Hook（首帧画面 + 首句口播/声音动作 + 产品出现时刻 + 可见证明），按停滑力、产品清晰度、可见证明、印尼语自然度、H3 可执行性和合规性评分，保留两条 A/B 变体。15 秒固定为 0–3 秒 Hook、3–10 秒一次真实演示/证据、10–15 秒产品细节→稳定 hero→单一 CTA；一个主要地点、一位成年人物、一个核心卖点。
脚本必须把 Bahasa 口播与 Bahasa 字幕分开输出：口播是演员实际说的完整句子，字幕是更短的屏幕文本；品牌名、SKU、数字和包装原文保持一致，字幕优先后期叠加。
写 H3 prompt 前必须调用 canvas_get_video_capabilities；完整英文提示词 90–170 词，直接可复制给 H3，并在单段内保留 0.0–3.0s HOOK、3.0–10.0s EVIDENCE、10.0–15.0s CONVERSION，明确参考图职责、产品轮廓/颜色/logo/标签/数量/比例锁定、开始状态→一条动作链→结束状态、字幕安全区、镜头/接触/物理约束、失败约束和原样印尼语口播。只在准备节点前展示 Product Truth Sheet、摘要、Prompt、口播、字幕和 A/B 变体，客户明确确认后才调用 canvas_prepare_video(confirmed=true)，绝不自动提交生成。
成片必须完整观看并听完整音频；逐项记录产品一致性、文字清晰度、Bahasa 口播自然度、违禁/高风险功效词、首帧吸引力和三段式 CTA，并附时间码证据。产品身份漂移、噪音/缺音、错语言、乱码、畸形、不可解码、时长/比例错误或虚假声明直接重跑。80 分以上且无硬失败才通过；每次只修一类失败，给出可直接复制的英文定向重跑 prompt 并保留产品锁。竞品只提取 Hook/节奏/证明结构，不复制原句、人物、音乐、品牌视觉或完整镜头顺序。
