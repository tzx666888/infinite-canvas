...
function buildExactCommerceVisualHook(direction: string, duration: number, productOnly: boolean, promptRoute: VideoPromptDetail) {
    ...
    const cleanCutReveal = `${secondBeatEnd}-${hookEndLabel}s: after one HARD EDITORIAL CUT with zero in-between transformation frames, show the original referenced product unit and any referenced retail box already fully formed, upright, and separately placed on a clean hero surface. Keep a persistent visible background gap between their silhouettes; the earlier stunt parcel or prop is absent.`;
    const humanHooks = [
        `0.00-${firstBeatEnd}s: begin mid-incident as a tall stack of empty lightweight shipping cartons topples toward an adult and freezes centimeters away while the adult ducks. ${firstBeatEnd}-${secondBeatEnd}s: the unchanged empty cartons snap backward in reverse and scatter safely out of frame without becoming another object. ${cleanCutReveal}`,
        `0.00-${firstBeatEnd}s: a sealed shipping parcel slams onto a theatrical breakaway cardboard display with a sharp impact; the adult is already recoiling in frame, never walking in or greeting. ${firstBeatEnd}-${secondBeatEnd}s: the same sealed rigid parcel rebounds upward against gravity and the adult catches it in surprise; it never opens or changes shape. ${cleanCutReveal}`,
        `0.00-${firstBeatEnd}s: begin with an adult already dropping from just above the top edge in a safe studio gravity gag, landing immediately on an oversized soft crash mat beside a sealed shipping parcel; no stairs, height, injury, or imitable danger. ${firstBeatEnd}-${secondBeatEnd}s: the landing launches the same sealed rigid parcel harmlessly upward and the adult catches it with a genuine shocked reaction; it stays closed and unchanged. ${cleanCutReveal}`,
        `0.00-${firstBeatEnd}s: an enormous parcel shadow races across the room while a tiny sealed shipping box drops into the adult's open hands. ${firstBeatEnd}-${secondBeatEnd}s: the same small rigid box bounces once as the adult reacts; it remains sealed, keeps exactly the same size, and never unfolds or transforms. ${cleanCutReveal}`,
    ];
    const productOnlyHooks = [
        `0.00-${firstBeatEnd}s: a tall stack of empty lightweight cartons topples toward the camera and freezes centimeters from the lens. ${firstBeatEnd}-${secondBeatEnd}s: every unchanged empty carton snaps backward in reverse and scatters safely out of frame without becoming another object. ${cleanCutReveal}`,
        ...
        `0.00-${firstBeatEnd}s: an enormous parcel shadow sweeps over an empty set while a tiny sealed shipping box drops into frame. ${firstBeatEnd}-${secondBeatEnd}s: the same small rigid box bounces once, remains sealed, keeps exactly the same size, and never unfolds or transforms. ${cleanCutReveal}`,
        `0.00-${firstBeatEnd}s: a fast domino chain of harmless empty props races toward an empty pedestal and stops one millimeter before impact. ${firstBeatEnd}-${secondBeatEnd}s: the unchanged props reverse at double speed and clear the empty pedestal; no product is present during this motion. ${cleanCutReveal}`,
    ];
