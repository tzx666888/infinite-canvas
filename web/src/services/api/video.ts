...
function buildExactCommerceVisualHook(direction: string, duration: number, productOnly: boolean, promptRoute: VideoPromptDetail) {
    ...
    const humanHooks = [
        ...
        `0.00-${firstBeatEnd}s: begin mid-motion in the reference-compatible everyday setting as an adult performs an obviously staged, harmless low slide or stumble and instantly counterbalances; no entrance, greeting, normal walk, height, stairs, traffic, or injury. ${firstBeatEnd}-${secondBeatEnd}s: use fast beat-synchronized HARD EDITORIAL CUTS to alternate the continuing safe recovery with two ultra-short macro flashes of the same original referenced product or finished food hero, already fully formed and correctly shaped in every flash; these are separate shots only, never a morph or shared frame. ${secondBeatEnd}-${hookEndLabel}s: return to the adult completing the safe recovery, add one final clean product or food macro flash on the strongest beat, then end the incident with no pain or damage. At exactly ${hookEndLabel}s, hard-cut to a stable readable hero shot of that same fully formed referenced product or finished food, with any retail package separately placed and a visible background gap between silhouettes.`,
    ];
    ...
    const allowsProductFlashCuts = !productOnly && hookIndex === humanHooks.length - 1;
    return [
        ...
        `RIGID-BODY SEPARATION LOCK — MANDATORY: product unit, bottle, cap, retail box, shipping parcel, platform, and hands are distinct rigid bodies with independent closed contours. Product and retail box must never share a mesh, surface, edge, label, or shadow and must never touch, fuse, bridge, melt, stretch, intersect, penetrate, or grow through one another. No product emerges from, grows out of, pops from, or transforms from any package. ${allowsProductFlashCuts ? "Any product or finished-food hook flash is a separate hard-cut shot of the already complete reference identity; never blend it with the incident frame. After the hook, hold a stable hero shot with visible background separation." : "Reveal it only after the hard cut, already complete and separated by visible background."}`,
...
function buildCommerceDramaVideoGuidance(direction: string, duration: number, productOnly = false, promptRoute: VideoPromptDetail = "short", hasExactVisualHook = false) {
    ...
    const hookBeatPlan =
        hookEnd >= 3
            ? "Hook timing lock: 0-1s trigger the expectation break, 1-2s escalate it, and 2-3s deliver the incident payoff. At exactly 3.00s, hard-cut to the stable readable product reveal. Brief editorial product flashes are allowed before 3s only when the earlier exact visual hook timeline explicitly requires them; never begin the stable product demonstration early, finish the hook early, or hold a static frame before 3s."
            : hookEnd === 2
              ? "Hook timing lock: 0-0.7s trigger the expectation break, 0.7-1.4s escalate it, and 1.4-2s deliver the incident payoff. At exactly 2.00s, hard-cut to the stable readable product reveal. Brief editorial product flashes are allowed before 2s only when the earlier exact visual hook timeline explicitly requires them; never begin the stable product demonstration early, finish the hook early, or hold a static frame before 2s."
