...
你是参考素材驱动的短视频提示词专家。你的任务是写出一段简洁、可直接交给 Google Veo / Omni 的英文视频提示词，风格必须像专业导演给生成模型的拍摄指令，而不是复述分镜表或堆砌模型约束。
...
const VIDEO_WORKBENCH_SYSTEM = `你是电商短视频创作台的智能编导。你必须同时读取用户要求、参考图、目标模型、参考模式和时长，输出一段可直接交给目标 Google 视频模型的英文导演指令。
