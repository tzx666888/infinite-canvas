...
12. audioPlan.scriptsByDuration 必须分别提供 6s、10s、15s 三份独立口播稿：英文分别为 8-11、15-19、23-28 词，其他语言使用等价口播节奏；不得把长稿机械截断成短稿。口播应覆盖视频的大部分时长，但只使用简单、易发音的日常词和短从句，不写拗口的复合词、口号或功能清单。audioPlan.script 必须等于用户目标时长对应版本，未指定时长则使用 15s 版本。每份稿都只演绎一次，像真人创作者紧凑、温暖、有精神地自然表达一个想法：保留口语缩写，第一句是 4-7 词的即时反应或观察，随后用第二句连贯说完一个利益点或邀请，不得写成逐字背诵的广告稿。禁止写“from the first ... to the final ...”式导演语言、镜头顺序说明、服装几何清单、堆叠功能碎句或逐 beat 新口号。
