...
export function CanvasConfigNodePanel({ node, isRunning, inputSummary, onConfigChange, onGenerate, onStop, onComposerToggle }: CanvasConfigNodePanelProps) {
    ...
    const hasVideoModels = globalConfig.videoModels.length > 0;
    const mode = node.metadata?.generationMode === "video" && !hasVideoModels ? "image" : node.metadata?.generationMode || "image";
    ...
    return (
        <div className="flex h-full w-full cursor-move flex-col px-3 pb-3 pt-7 text-sm" style={{ color: theme.node.text }} onWheel={(event) => event.stopPropagation()}>
            ...
            {videoRoute ? (
                <div className={`mb-2 cursor-default text-[11px] leading-4 ${videoRoute.error ? "text-red-400" : ""}`} style={videoRoute.error ? undefined : { color: theme.node.muted }} onMouseDown={(event) => event.stopPropagation()}>
                    本次：{videoRoute.text}
                </div>
            ) : null}
...
function buildNodeConfig(globalConfig: AiConfig, node: CanvasNodeData, mode: CanvasGenerationMode): AiConfig {
    ...
    return {
        ...
        videoPromptMode: node.metadata?.videoPromptMode || globalConfig.videoPromptMode || defaultConfig.videoPromptMode,
...
function defaultModelForMode(config: AiConfig, mode: CanvasGenerationMode) {
    return mode === "image" ? config.imageModel : mode === "video" ? config.videoModel : mode === "audio" ? config.audioModel : config.textModel;
}
