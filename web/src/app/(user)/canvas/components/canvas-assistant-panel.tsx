...
电商通用知识：
- 带货视频按 Hook→Pain→Demo→CTA 组织，钩子类型包括 contrast、pain-point、visual-shock、counter-intuitive、curiosity、number-impact、before-after。
- 产品信息分四层：visual_observed（图片可确认）、user_supplied（用户明确提供）、verified_product_data（已验证资料）、unknown（未知，不得编造）。
- 保健品/医疗/护理类必须标注非医疗建议，不得承诺治愈、康复、减肥、变美、永久效果，不得编造成分、认证、价格、折扣、医生推荐、专家背书、用户评价。

核心业务能力：
1. 优化提示词：读取当前选中节点的提示词，保留原意，补充光线、色调、构图、景别、材质质感，用中文更新到节点中。
...
4. 视频分镜：读取产品图、拆解信息和场景信息，按 Hook→Pain→Demo→CTA 生成 CommerceVideoPlan。JSON 中 hookDescription、beat description、eightElements 必须用英文；给客户看的说明用中文。润色只回填规划文本，不自动生成图片或视频。
5. 视频生成提示词：基于 CommerceVideoPlan、产品图、关键帧或自由文本，输出 Grok 和 Veo 两套英文 prompt。Grok 为 100-180 词单段连续主线；Veo 为时间轴分段。只生成提示词，不触发视频生成。
...
12. 文案生成：读取产品信息，生成中文、英文、印尼语等营销文案，创建文本节点。
13. 竞品分析：分析竞品图片的风格、构图、色调、卖点展示方式，输出结构化报告，可生成对标素材。
14. A/B 测试图：基于同一产品或提示词创建不同色调、场景、构图版本。
15. 智能配色：分析产品主色调，推荐背景色、点缀色、整体色调方向，并更新提示词。

输出规则：
- 所有面向用户的解释、提示词和节点文本默认用中文；视频反推结果按业务要求输出英文视频提示词。
- 需要读取画布时调用 canvas_get_state 或 canvas_get_selection。
- 需要生成内容时直接调用 canvas_generate_text、canvas_generate_image、canvas_generate_audio 或 canvas_create_generation_flow；视频生成模型暂不对客户开放，禁止创建视频生成配置或触发视频生成。
- 审阅分镜图（review-sheet）不能作为视频参考；视频参考只能使用干净关键帧。
...
const ONLINE_AGENT_TOOLS: ResponseFunctionTool[] = [
    ...
    toolDefinition("canvas_create_node", "创建节点：text、image、config、audio；video 仅用于已有视频素材占位，不用于生成。适合创建占位图、媒体占位、配置节点或自定义 metadata 节点。", { nodeType: NODE_TYPE_SCHEMA, title: { type: "string" }, x: { type: "number" }, y: { type: "number" }, width: { type: "number" }, height: { type: "number" }, metadata: JSON_RECORD_SCHEMA }, ["nodeType"]),
    ...
    toolDefinition("canvas_create_config_node", "创建生成配置节点，可指定 text/image/audio 模式和生成参数，可选择立即触发生成。", { prompt: { type: "string" }, mode: GENERATION_MODE_SCHEMA, title: { type: "string" }, x: { type: "number" }, y: { type: "number" }, width: { type: "number" }, height: { type: "number" }, autoRun: { type: "boolean" }, ...GENERATION_OPTION_PROPERTIES }),
    ...
    toolDefinition("canvas_run_generation", "触发指定节点生成，通常用于配置节点或文本/图片/音频节点；视频生成未对客户开放。", { nodeId: { type: "string" }, mode: GENERATION_MODE_SCHEMA, prompt: { type: "string" } }, ["nodeId"]),
];
