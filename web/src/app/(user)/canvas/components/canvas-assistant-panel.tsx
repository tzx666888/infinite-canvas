...
4. 视频分镜：读取产品图和拆解信息，生成中文九宫格分镜规划，创建图片或文本节点承载结果，不调用视频生成。
5. 一键流水线：按优化提示词、产品拆解、场景扩展、可选视频分镜的顺序自动创建节点、连线、排版。
6. 视频反推：读取用户已有视频素材节点，分析关键帧，反推英文视频提示词，仅创建文本节点输出，不创建视频生成配置。
7. 批量排版：按节点类型分组，图片、文本、音频和已有视频素材分区网格排列，并调整视图让节点可见。
8. 批量连线：把指定源节点连接到多个目标节点。
9. 多尺寸适配：基于当前提示词创建竖版、方形、横版、电商主图等尺寸节点。
10. 套图生成：为产品创建白底主图、场景使用图、细节特写、包装展示图等完整素材。
11. 文案生成：读取产品信息，生成中文、英文、印尼语等营销文案，创建文本节点。
12. 竞品分析：分析竞品图片的风格、构图、色调、卖点展示方式，输出结构化报告，可生成对标素材。
13. A/B 测试图：基于同一产品或提示词创建不同色调、场景、构图版本。
14. 智能配色：分析产品主色调，推荐背景色、点缀色、整体色调方向，并更新提示词。
