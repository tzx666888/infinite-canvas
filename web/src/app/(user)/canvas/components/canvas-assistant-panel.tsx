...
- 产品信息分四层：visual_observed（图片可确认）、user_supplied（用户明确提供）、verified_product_data（已验证资料）、unknown（未知，不得编造）。
- 对参考图产品的 visual_observed 身份描述，保留颜色类别、材质与表面质感、部件结构与相对位置、有无 logo/文字、道具与产品的区分；禁止仅凭图片作绝对量化判断，包括但不限于“高筒/矮胖/细长/宽扁”等比例断言、“高饱和/低饱和”等饱和度断言，以及“六边形/八边形”等具体边数断言。
- 对比例、饱和度和具体多边形边数改用相对锚定表述，例如“杯身高宽比与参考图完全一致”“颜色与参考图完全一致的青绿色”“多边形手柄外形与参考图完全一致”。画面中清晰可数且不超过 5 个的特征允许写明确数量；超过 5 个或无法清晰计数时写“数量与参考图一致”。
- 不得编造功效、成分、认证、价格、折扣、销量、医生推荐、用户评价或不可验证的前后对比。医疗、保健、护理、美妆和食品只展示可观察的使用过程。
- 参考图是身份权威；人物、产品、服装、logo、标签位置、现实尺寸与部件结构必须保持。泳装或贴身服装只采用明确成年人的非色情时尚编辑/商品目录语境。
...
11. 套图生成：为产品创建白底主图、场景使用图、细节特写、包装展示图等完整素材。
12. 文案生成：读取产品信息，按用户明确指定的目标语言生成营销文案；未指定地域时不预设单一国家或语言，创建文本节点。
13. 竞品分析：分析竞品图片的风格、构图、色调、卖点展示方式，输出结构化报告，可生成对标素材。
14. A/B 测试图：基于同一产品或提示词创建不同色调、场景、构图版本。
15. 智能配色：分析产品主色调，推荐背景色、点缀色、整体色调方向，并更新提示词。
16. 全球商业强 Hook 导演：当用户要求带货视频、UGC、强 Hook、MiniMax H3 提示词或成片质检时触发。先基于当前商品图建立 Product Truth Sheet，区分 visual_observed / user_supplied / verified_product_data / unknown，并提取品类、完整品名、SKU/规格/颜色/口味/数量、包装结构、logo/标签位置和逐字包装文字；模糊文字写“包装文字不可辨”，多个 SKU、资料矛盾、语言变体不明或卖点无证据时先提问，不编造功效、价格、折扣、销量、评价、库存紧迫感或前后对比。输出 3 个结构明显不同的 Hook，可从临界预期反转、爆裂显形、尺度反差、空间错位、错误结果反转、反直觉运动、安全的超现实坠落反转，以及具有脚步、重心和停止过程的连续失衡动作后一次甩镜揭示中选择，按停滑力、产品清晰度、模型可执行性、安全性和地域适配评分并保留两条 A/B 变体，不得反复套同一种事故。15 秒使用 0–3 秒 Hook、3–10 秒一次真实演示/证据、10–15 秒产品细节→稳定 hero→单一 CTA；6 秒和 10 秒按比例压缩。只有用户明确指定国家、城市、语言、货币或平台时才一致本地化；未指定时保持全球中性，不预设任何固定地域、语言、货币、地标或平台徽标。人物危险动作只能明显舞台化或超现实、无伤害、无血腥、不可模仿；商品不得导致、参与或成为危险行为的工具；失衡 Hook 中不得来回插入商品或重置人物姿态，商品只在动作完整收住后通过一次硬切揭示。用户“不要人物、烟雾、旋转”等硬约束永远高于 Hook 模板。写视频提示词前调用 canvas_get_video_capabilities，完整英文 prompt 90–170 词，直接可复制给当前模型并保留产品身份锁、动作链、字幕安全区、身份/物理/音频与失败约束。成片任务必须完整观看并听完整音频，逐项检查产品一致性、文字清晰度、口播自然度、硬约束、违禁功效词、首帧吸引力和 CTA；硬失败直接重跑，每次只修一类问题并给英文定向重跑 prompt。竞品只提取结构，不复制原句、人物、音乐、品牌视觉或完整镜头顺序。
...
- 提示词正文写成 45–85 个英文词的一条连续创作指令，由系统补齐模型、参考图角色和身份约束后，最终发给视频模型的完整提示词必须保持 90–170 个英文词：一个清晰主体、一处主要场景、连续动作、镜头与光线、真实产品交互。10 秒内不超过 3 个可见节拍，不塞入多地点长剧情；一般视频不得包含标题、Markdown、复杂时间表、data URL 或 base64，全球商业强 Hook 导演在 15 秒成片方案中可保留 0.0–3.0s / 3.0–10.0s / 10.0–15.0s 三段时间边界。
...
function generationToolDefinition(name: string, description: string, mode?: "text" | "image" | "audio") {
...
const ONLINE_AGENT_TOOLS: ResponseFunctionTool[] = [
    ...
    toolDefinition("canvas_create_config_node", "创建生成配置节点，可指定 text/image/audio 模式和生成参数，可选择立即触发生成。", {
        prompt: { type: "string" },
    ...
    generationToolDefinition("canvas_create_generation_flow", "创建通用生成流程：提示词文本节点、生成配置节点、参考节点连线，可用于文案、生图或音频。"),
                    ...
                    {view === "history" ? (
                        ...
                        <div className="flex h-full flex-col items-center justify-center px-1 text-center">
                            <div className="relative font-serif text-4xl font-bold italic tracking-normal" style={{ color: theme.node.text }}>
                                <span>视觉画布</span>
                                <DiaTextReveal className="absolute inset-0" colors={["#22D3EE", "#3B82F6", "#A855F7"]} textColor="transparent" duration={1.8} startOnView={false} text="视觉画布" />
                            </div>
                            <div className="mt-3 text-xs font-medium uppercase tracking-[0.28em] opacity-60">Visual Canvas</div>
                        </div>
                    )}
                </div>
            )}
...
function defaultGenerationModel(config: AiConfig, mode: "text" | "image" | "audio") {
...
function resolveGenerationModel(config: AiConfig, mode: "text" | "image" | "audio", model?: string) {
...
function compactMetadata(metadata: CanvasNodeData["metadata"]) {
    return {
        ...
        size: metadata?.size,
        productBreakdownPlan: compactProductBreakdownPlan(metadata?.productBreakdownPlan),
        sceneExpansionPlan: compactSceneExpansionPlan(metadata?.sceneExpansionPlan),
        commerceVideoPlan: compactCommerceVideoPlan(metadata?.commerceVideoPlan),
        selectedHookType: metadata?.selectedHookType,
        excludedHookTypes: metadata?.excludedHookTypes,
        storyboardRole: metadata?.storyboardRole,
        storyboardBeatIndex: metadata?.storyboardBeatIndex,
        targetVideoModel: metadata?.targetVideoModel,
        targetVideoSeconds: metadata?.targetVideoSeconds,
        targetVideoSize: metadata?.targetVideoSize,
    };
}

function compactProductBreakdownPlan(plan: NonNullable<CanvasNodeData["metadata"]>["productBreakdownPlan"]) {
    if (!plan) return undefined;
    return {
        productName: plan.productName,
        category: plan.category,
        identity: plan.identity,
        materials: plan.materials?.slice(0, 8),
        components: plan.components?.slice(0, 8),
        visibleMarks: plan.visibleMarks?.slice(0, 8),
        packageAccessories: plan.packageAccessories?.slice(0, 8),
        shots: plan.shots?.slice(0, 8).map((shot) => ({
            title: shot.title,
            focus: shot.focus,
        })),
    };
...
function compactSceneExpansionPlan(plan: NonNullable<CanvasNodeData["metadata"]>["sceneExpansionPlan"]) {
    if (!plan) return undefined;
    return {
        productName: plan.productName,
        identity: plan.identity,
        scenes: plan.scenes?.slice(0, 10).map((scene) => ({
            title: scene.title,
            focus: scene.focus,
        })),
    };
...
function compactCommerceVideoPlan(plan: NonNullable<CanvasNodeData["metadata"]>["commerceVideoPlan"]) {
    if (!plan) return undefined;
    return {
        productCategory: plan.productCategory,
        selectedHookType: plan.selectedHookType,
        hookDescription: plan.hookDescription,
