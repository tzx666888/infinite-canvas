...
import { App, Button, Dropdown, Input, Segmented, Tooltip } from "antd";
import copyToClipboard from "copy-to-clipboard";
import { ChevronDown, Copy, FolderOpen, History, KeyRound, Link2, PlugZap, Plus, RefreshCw, RotateCcw, Settings2, Terminal, Trash2 } from "lucide-react";
...
const AGENT_CONNECT_STEPS = [
    { title: "1. 准备本地 AI 助手", text: "先确认本机 AI 助手环境可以正常使用，并已授权访问需要的工具。" },
...
export function CanvasLocalAgentPanel({
    ...
    useEffect(() => {
        onPrepareAgentVideoRef.current = onPrepareAgentVideo;
    }, [onPrepareAgentVideo]);
    useEffect(() => {
    ...
    const handleToolCall = async (endpoint: string, token: string, payload: AgentPendingToolCall) => {
        if (confirmToolsRef.current && (payload.name === "canvas_apply_ops" || payload.name === "canvas_prepare_video")) {
    ...
    const runToolCall = async (endpoint: string, token: string, payload: AgentPendingToolCall) => {
        try {
            const input: Record<string, unknown> & { ops?: CanvasAgentOp[] } = payload.input || {};
            ...
            if (payload.name === "canvas_update_video_brief") {
            ...
            if (payload.name === "canvas_prepare_video") {
    ...
    const switchAgentTab = (activeTab: AgentPanelTab) => {
    ...
    const content = (
        <>
            <div className="border-b px-3 py-2.5" style={{ borderColor: theme.node.stroke }}>
                <div className="flex min-w-0 items-center justify-between gap-3">
                    <button type="button" className="min-w-0 text-left" onClick={() => switchAgentTab("chat")}>
                        <div className="truncate text-sm font-semibold leading-5">AI 助手</div>
                        <div className="mt-0.5 truncate text-[11px] leading-4" style={{ color: connected ? theme.node.muted : theme.node.faint }}>
                            {connected ? activity || "已连接" : "本地助手未连接"}
                        </div>
                    </button>
                        ...
                        <Dropdown
                            ...
                            menu={{
                                ...
                                items: [
                                    { key: "chat", label: "对话" },
                                    { type: "divider" },
                                    { key: "setup", label: "连接配置", icon: <PlugZap className="size-3.5" /> },
                                    { key: "history", label: `历史${threads.length ? ` ${threads.length}` : ""}`, icon: <History className="size-3.5" /> },
                                    { key: "log", label: `日志${eventLogs.length ? ` ${eventLogs.length}` : ""}`, icon: <Terminal className="size-3.5" /> },
                                ],
                                onClick: ({ key }) => switchAgentTab(key as AgentPanelTab),
                            }}
                        >
                            <Button size="small" icon={<Settings2 className="size-3.5" />}>
                                高级
...
const agentStarterPrompts = ["帮我整理当前画布，把素材按生成链路排好", "检查失败节点，告诉我应该怎么重试", "把选中的图片延展成 3 个商品图方向"];

function AgentChatStarter({ theme, connected, onPick, onOpenSetup }: { theme: (typeof canvasThemes)[keyof typeof canvasThemes]; connected: boolean; onPick: (prompt: string) => void; onOpenSetup: () => void }) {
    return (
        <div className="rounded-xl border p-3" style={{ borderColor: theme.node.stroke, background: theme.toolbar.panel }}>
            <div className="text-sm font-medium leading-5">可以直接这样问</div>
            <div className="mt-3 flex flex-wrap gap-2">
                {agentStarterPrompts.map((prompt) => (
                    <button key={prompt} type="button" className="rounded-full border px-3 py-1.5 text-xs leading-5 transition hover:opacity-90" style={{ borderColor: theme.node.stroke, color: theme.node.text }} onClick={() => onPick(prompt)}>
                        {prompt}
                    </button>
                ))}
            </div>
            {!connected ? (
                <Button className="mt-3 !h-8" size="small" type="primary" icon={<PlugZap className="size-3.5" />} onClick={onOpenSetup}>
                    连接本地助手
                </Button>
            ) : null}
        </div>
    ...
    return (
        <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
            <div className="space-y-4">
                <div>
                    <div className="text-base font-semibold leading-6">连接本地 AI 助手</div>
                    <div className="mt-1 text-xs leading-5" style={{ color: theme.node.muted }}>
                        本地服务只监听 127.0.0.1，画布通过地址和 token 连接到你电脑上的 AI 助手。
                    </div>
                </div>
...
function formatAgentEvent(event: AgentEventPayload): Omit<AgentChatItem, "id"> | null {
    ...
    if ((event.type === "item.updated" || event.type === "item.completed") && item?.type === "agent_message") return { role: "assistant", title: "本地 AI 助手", text: stringText(item.text), meta: usageText(event), streamId: item.id };
    ...
    if (text) return { role: "assistant", title: "本地 AI 助手", text, meta: usageText(event) };
    return null;
}
...
function eventTitle(event: AgentEventPayload) {
    const item = event.item;
    if (event.type === "thread.started") return "已创建本地会话";
    ...
    if (event.type === "item.completed" && item?.type === "agent_message") return "本地 AI 助手回复";
    return event.type || "本地 AI 助手事件";
}
...
function localVideoBrief(value: unknown): Partial<CanvasAgentVideoBrief> {
    return Object.fromEntries(
        Object.entries({
            productNodeId: stringText(objectField(value, "productNodeId")).trim() || undefined,
            creatorNodeId: stringText(objectField(value, "creatorNodeId")).trim() || undefined,
            videoType: stringText(objectField(value, "videoType")) || undefined,
            market: stringText(objectField(value, "market")).trim() || undefined,
            platform: stringText(objectField(value, "platform")).trim() || undefined,
            language: stringText(objectField(value, "language")).trim() || undefined,
            model: stringText(objectField(value, "model")).trim() || undefined,
            seconds: typeof objectField(value, "seconds") === "number" ? objectField(value, "seconds") : undefined,
            size: stringText(objectField(value, "size")) || undefined,
            generateAudio: typeof objectField(value, "generateAudio") === "boolean" ? objectField(value, "generateAudio") : undefined,
            withSubtitle: typeof objectField(value, "withSubtitle") === "boolean" ? objectField(value, "withSubtitle") : undefined,
            sellingPoint: stringText(objectField(value, "sellingPoint")).trim() || undefined,
