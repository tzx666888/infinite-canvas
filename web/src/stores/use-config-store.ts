...
import { TOKAXIS_VIDEO_ENHANCER_MODEL_IDS } from "@/lib/aliyun-video-enhancer";
...
const TOKAXIS_FALLBACK_MODELS = [
    ...
    ...TOKAXIS_VIDEO_ENHANCER_MODEL_IDS,
...
const TOKAXIS_VIDEO_MODEL_IDS = new Set<string>([...GOOGLE_VIDEO_MODEL_IDS, ...TOKAXIS_MINIMAX_H3_VIDEO_MODEL_IDS.map((model) => model.toLowerCase()), ...TOKAXIS_VIDEO30_MODEL_IDS.map((model) => model.toLowerCase()), ...TOKAXIS_VIDEO_ENHANCER_MODEL_IDS.map((model) => model.toLowerCase())]);
...
export const defaultConfig: AiConfig = {
    ...
    textModel: `tokaxis::${TOKAXIS_AGENT_TEXT_MODEL_IDS[0]}`,
...
type ConfigStore = {
    ...
    setPlatformApiKey: (apiKey: string, ownerUserId?: string) => void;
...
export const useConfigStore = create<ConfigStore>()(
    persist(
        (set, get) => ({
            ...
            setPlatformApiKey: (apiKey, ownerUserId) =>
                set((state) => ({
                    config: {
                        ...state.config,
                        platformApiKeyOwnerId: ownerUserId,
                        apiKey: apiKey.trim(),
...
function buildTokaxisConfigWithModels(config: AiConfig, apiKey: string, rawModels: string[]): AiConfig {
    ...
    return {
        ...
        imageModel: normalizeDefaultTokaxisImageModel(config.imageModel || config.model, config.size, false, capabilityLists.imageModels, channels),
...
function sanitizeTokaxisModels(models: string[]) {
    const visibleModels = uniqueRawModels(models).filter(
        (model) => !TOKAXIS_REMOVED_MODEL_IDS.has(modelOptionName(model).trim().toLowerCase()) && !isDisabledModelName(model) && !TOKAXIS_DISABLED_IMAGE_MODEL_RE.test(model) && (!isImageModelName(model) || TOKAXIS_PUBLIC_IMAGE_MODEL_IDS.has(model)),
    );
