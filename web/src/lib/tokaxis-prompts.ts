...
export const TOKAXIS_PROMPTS: Prompt[] = [
    ...
    tokaxisPrompt({
        ...
        title: "全球商业强 Hook 导演",
        summary: "按目标市场自动本地化，生成多种反直觉强 Hook、三段式商业视频和成片质检。",
        prompt: "请启动全球商业强 Hook 导演技能。先读取画布中的商品参考图与资料，建立 Product Truth Sheet，锁定商品外形、颜色、logo、标签、数量和比例；不确定时先提问，禁止编造。先提出 3 个不同 Hook 并按停滑力、商品清晰度、模型可执行性、安全性和地域适配评分，优先从临界预期反转、爆裂显形、尺度反差、空间错位、错误结果反转、反直觉运动，以及具有脚步、重心和停止过程的连续失衡动作后一次甩镜揭示中选，不要反复套同一种事故。15 秒按 0–3 秒 Hook、3–10 秒证据/演示、10–15 秒商品 hero 与单一 CTA，6 秒和 10 秒按比例压缩。只有我明确指定国家、城市、语言、货币或平台时才做对应本地化；未指定时保持全球中性，不预设印尼、美国或任何固定地域。人物危险动作只能采用明显舞台化、超现实、无伤害、无血腥的表现，商品不得成为危险行为的原因或工具；失衡 Hook 中不得来回插入商品或重置人物姿态，商品只在动作完整收住后通过一次硬切揭示。输出可直接投喂当前视频模型的英文 prompt；生成后检查商品一致性、文字清晰度、口播自然度、硬约束、首帧吸引力和 CTA，并给出定向重跑建议。商品/目标：",
        tags: ["全球商业", "强Hook", "爆款视频", "动态本地化", "成片质检"],
        category: "Tokaxis 创作",
