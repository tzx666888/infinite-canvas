...
export type TokaxisAuthResult = { status: "authenticated"; identity: TokaxisIdentity } | { status: "invalid" } | { status: "two_factor_required"; message: string } | { status: "blocked"; message: string } | { status: "unavailable" };

function tokaxisOrigin() {
    return (process.env.CANVAS_LEGACY_AUTH_ORIGIN || process.env.TOKAXIS_INTERNAL_ORIGIN || "http://127.0.0.1:3000").replace(/\/+$/, "");
}
