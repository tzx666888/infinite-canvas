...
assert.deepEqual(fixedGoogleVideoDurationOptions(t2v), [8]);
...
assert.equal(normalizeGoogleVideoSeconds("16", t2v), "8");
assert.equal(normalizeGoogleVideoSeconds("16", r2v), "8");
assert.equal(normalizeGoogleVideoSeconds("15", "omni"), "10");
assert.equal(isGoogleVeoOfficialExtendDuration("15", t2v), false);
