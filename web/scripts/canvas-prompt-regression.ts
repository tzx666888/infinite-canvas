...
assert.equal(buildImageReferencePromptText(configNodeCommercePrompt, targetReference), "参考图片按上传顺序固定编号为：图片1。\n参考连线执行顺序是强制性的：严格按 1 → 2 → 3（以实际图片数量为准）推进，不得自行重排。\n- 第1步：先执行图片1，完成该图片指定的主体/场景/构图作用后，才能进入下一步。\n不得交换、合并、平均、混淆不同图片中的主体，也不得跳过任何已连接图片。\n\n生成一组不同场景电商详情图");
const orderedImagePrompt = buildImageReferencePromptText("按参考顺序合成一张商品图", [{ id: "image-1" }, { id: "image-2" }, { id: "image-3" }] as Parameters<typeof buildImageReferencePromptText>[1]);
assert.match(orderedImagePrompt, /参考连线执行顺序是强制性的/);
assert.match(orderedImagePrompt, /第1步：先执行图片1/);
assert.match(orderedImagePrompt, /第2步：先执行图片2/);
assert.match(orderedImagePrompt, /第3步：先执行图片3/);
assert.match(orderedImagePrompt, /不得交换、合并、平均、混淆/);
