...
assert.match(videoGenerationPreflight, /const deliverySize = facebookMediaPreset\(config\.size\)\?\.id/);
assert.equal((videoGenerationPreflight.match(/size: deliverySize \|\|/g) || []).length, 5);
assert.match(videoGenerationPreflight, /product\?\.family === "omni"/);
