...
assert.match(cleanAnchorPrompt, /Say once, clearly and conversationally/i);
...
assert.match(cleanAnchorPrompt, /Do not fill 15s with speech; after the line, continue beach ambience and low music/i, "creator speech must end in a deliberate product hold instead of being stretched across the clip");
assert.doesNotMatch(cleanAnchorPrompt, /finish at 14\.4-14\.8s|175-185 wpm|finish near 13s/i);
...
assert.match(mixedCleanerPrompt, /One quick spray on my black bikini/i);
...
assert.match(mixedCleanerPrompt, /Do not fill 15s with speech; after the line, continue beach ambience and low music/i);
...
assert.match(repairedStalledCreatorPrompt, /Say once, clearly and conversationally/i);
...
assert.match(repairedStalledCreatorPrompt, /Do not fill 15s with speech/i);
...
assert.match(videoPromptCompilerSource, /Do not fill 15s with speech; after the line, continue beach ambience and low music/, "15-second creator speech must end with an intentional product hold");
