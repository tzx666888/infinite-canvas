...
assert.match(cleanAnchorPrompt, /brisk 175-185 wpm, one breath under 0\.4s, no other gaps/i, "creator speech must avoid long or scattered pauses");
assert.match(cleanAnchorPrompt, /finish near 13s, then silently hold product over ambience and low music/i, "creator speech must end in a deliberate product hold instead of being stretched across the clip");
assert.doesNotMatch(cleanAnchorPrompt, /finish at 14\.4-14\.8s/i);
...
assert.match(mixedCleanerPrompt, /I spray my black bikini, clean the salty spots/i);
assert.doesNotMatch(mixedCleanerPrompt, /I'm cleaning the black bikini right here|spraying the removed|hard cut to/i);
assert.match(mixedCleanerPrompt, /brisk 175-185 wpm, one breath under 0\.4s, no other gaps, repeats, restarts, or stretched words; finish near 13s/i);
assert.match(mixedCleanerPrompt, /silently hold product over ambience and low music/i);
...
assert.match(repairedStalledCreatorPrompt, /finish near 13s/i);
assert.match(repairedStalledCreatorPrompt, /silently hold product over ambience and low music/i);
...
assert.match(videoPromptCompilerSource, /brisk 175-185 wpm, one breath under 0\.4s, no other gaps, repeats, restarts, or stretched words/, "15-second creator speech must preserve compact creator cadence");
assert.match(videoPromptCompilerSource, /finish near 13s, then silently hold product over ambience and low music/, "15-second creator speech must end with an intentional product hold");
