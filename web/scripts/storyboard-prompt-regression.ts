...
assert.match(cleanAnchorPrompt, /Say once in one connected conversational flow/i);
assert.match(cleanAnchorPrompt, /no restart, repeated phrase, invented filler, forced pause, or stretched word/i, "creator speech must reject repetition, gibberish, and time stretching");
assert.match(cleanAnchorPrompt, /only brief natural breaths/i, "creator speech must remain connected without imposing a synthetic reading speed");
assert.match(cleanAnchorPrompt, /Let natural location ambience and low music continue after the line ends/i, "creator speech must end in a deliberate product hold instead of being stretched across the clip");
...
assert.match(mixedCleanerPrompt, /I always keep this cleaner in my beach bag/i, "conflicting saved narration must be replaced by a physically coherent creator line");
assert.match(mixedCleanerPrompt, /one quick spray on my black bikini to clear the salty spots/i);
assert.doesNotMatch(mixedCleanerPrompt, /I'm cleaning the black bikini right here|spraying the removed|hard cut to/i);
assert.match(mixedCleanerPrompt, /no restart, repeated phrase, invented filler, forced pause, or stretched word/i);
assert.match(mixedCleanerPrompt, /Say once in one connected conversational flow/i);
assert.match(mixedCleanerPrompt, /Let natural location ambience and low music continue after the line ends/i);
assert.match(mixedCleanerPrompt, /Lip-sync the opening/i);
...
assert.match(repairedStalledCreatorPrompt, /Say once in one connected conversational flow/i);
...
assert.match(repairedStalledCreatorPrompt, /only brief natural breaths/i);
assert.match(repairedStalledCreatorPrompt, /Let natural location ambience and low music continue after the line ends/i);
...
assert.match(videoPromptCompilerSource, /no restart, repeated phrase, invented filler, forced pause, or stretched word/, "15-second creator speech must reject repetition, gibberish, and time stretching");
assert.match(videoPromptCompilerSource, /Say once in one connected conversational flow/, "creator speech must not be split into a presenter-style sentence schedule");
assert.match(videoPromptCompilerSource, /Let natural location ambience and low music continue after the line ends/, "15-second creator speech must end with an intentional product hold");
