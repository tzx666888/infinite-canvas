...
assert.equal(normalizeMiniMaxH3Duration(4), 4);
...
assert.match(serviceSource, /const promptText = facebookVideoSafeFramePrompt\(/);
assert.match(serviceSource, /limitVideoPrompt\(buildReferenceVideoPrompt\(prompt, references\.length, references\.length, duration/);
